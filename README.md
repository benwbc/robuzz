...
(the bucket name), and `CLIENT_ORIGIN` (fill this in after step 4, then redeploy). Deploy, then confirm it's up by
visiting `https://<your-api-name>.onrender.com/api/health` — it should show `{"ok":true,"db":"postgres"}`.
...
**5. Seed the live database once** — from your own machine, with the same `DATABASE_URL` from step 2:
...
This creates the tables and demo accounts. **Only run it once** — it wipes and rebuilds everything, so running it
again later would erase any real posts/accounts your live site has by then.

Open your Render static site's URL — that's your live BlockFeed.
