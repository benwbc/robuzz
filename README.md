...
`CLIENT_ORIGIN`, `SUPABASE_URL` and `SUPABASE_BUCKET` are trimmed and cleaned up automatically on boot (a pasted
trailing slash or stray extra path won't break anything — see `server/src/utils/normalize.js`), so pasting them
straight from Render/Supabase's own pages is enough.
