  ...
  return (
    <div>
      ...
      <div className="settings-section">
        <h3>Legal</h3>
        <Link to="/rules" className="link-button" style={{ display: 'inline-block' }}>
...
function ProfileSection({ user, updateUser }) {
  ...
  return (
    <form className="settings-section" onSubmit={save}>
      ...
      <div className="field">
        <label>Bio</label>
        <MentionTextarea as="textarea" rows={3} value={bio} onChange={(e) => setBio(e.target.value)} maxLength={160} />
      </div>
...
function AccountSection({ user, updateUser }) {
  ...
  const savePassword = async (e) => {
    ...
    try {
      ...
      if (!user.hasPassword) updateUser({ hasPassword: true }); // switches this form to the normal "change password" shape below
    } catch (err) {
    ...
    <>
      <form className="settings-section" onSubmit={saveUsername}>
        ...
        <div className="field">
          ...
          <input type="text" value={username} onChange={(e) => setUsername(sanitizeUsername(e.target.value))} maxLength={20} />
        </div>
        {user.hasPassword && (
          <div className="field">
      ...
      <form className="settings-section" onSubmit={savePassword}>
        ...
        {user.discordUsername && (
          <p className="hint">Signed in with Discord as @{user.discordUsername}.</p>
        )}
        {!user.hasPassword && (
          <p className="hint">You don't have a password yet — set one so you can also log in without Discord.</p>
        )}
        {passwordError && <div className="alert alert-error">{passwordError}</div>}
        {passwordSuccess && <div className="alert alert-success">Password {user.hasPassword ? 'updated' : 'set'}.</div>}
        {user.hasPassword && (
          <div className="field">
            <label>Current password</label>
        ...
        <button className="btn btn-primary" type="submit" disabled={passwordBusy}>
          {passwordBusy ? 'Saving…' : user.hasPassword ? 'Change password' : 'Set password'}
        </button>
      </form>
    </>
  );
