...
function EditUserModal({ user, onClose, onSaved }) {
  ...
  const removeAvatar = async () => {
    if (!confirm(`Remove @${user.username}'s profile picture?`)) return;
    try {
      const { user: u } = await api.removeAdminUserAvatar(user.id);
      setAvatarUrl(u.avatarUrl);
    } catch (err) {
      alert(err.message);
    }
  ...
  const uploadBanner = async (file) => {
    ...
    fd.append('image', file);
    try {
      const { user: u } = await api.uploadAdminUserBanner(user.id, fd);
  ...
  const removeBanner = async () => {
    if (!confirm(`Remove @${user.username}'s banner?`)) return;
    try {
      const { user: u } = await api.removeAdminUserBanner(user.id);
      setBannerUrl(u.bannerUrl);
    } catch (err) {
      alert(err.message);
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      onSaved({ ...updated, avatarUrl, bannerUrl });
    } catch (err) {
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="field">
          ...
          <div
            ...
            style={{ height: 90, borderRadius: 12, cursor: 'pointer', position: 'static' }}
          ...
          />
          {bannerUrl && (
            <button type="button" className="btn btn-secondary btn-sm" style={{ marginTop: 8 }} onClick={removeBanner}>
              Remove banner
            </button>
          )}
        </div>
