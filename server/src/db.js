...
// node-postgres returns COUNT(*) and other bigint columns (Postgres type
// OID 20) as JS strings by default, not numbers — a precaution against
// values too large for a JS number to represent exactly, which none of
// this app's counts (users, likes, unread notifications, ...) ever come
// close to. Left alone, every `SELECT COUNT(*) c FROM ...` in this codebase
// comes back as e.g. "0" instead of 0 in production (Postgres) while
// staying a real 0 locally (SQLite) — and "0" is truthy in JS, so anything
// checking a count like `if (count)` silently misbehaves only in
// production. Parsing bigint as a real integer here, once, up front,
// keeps both backends behaving identically everywhere they're compared.
if (isPostgres) pg.types.setTypeParser(20, (val) => parseInt(val, 10));
...
export async function initDb() {
  ...
  // Migrations for databases created before Discord sign-in existed. Every
  // pre-existing account signed up with a real password, so has_password
  // defaults to 1 (true) for everyone already in the table — only accounts
  // created through Discord going forward start at 0.
  await ensureColumn('users', 'discord_id', 'TEXT');
  await ensureColumn('users', 'discord_username', 'TEXT');
  await ensureColumn('users', 'has_password', 'INTEGER NOT NULL DEFAULT 1');
  // This index has to be created down here, after discord_id is guaranteed
  // to exist — SCHEMA above runs first and only CREATEs the table if it's
  // missing entirely, so on a database from before this column existed, an
  // index on it living up in SCHEMA would fail at startup with "no such
  // column" before ensureColumn ever got a chance to add it.
  await exec('CREATE INDEX IF NOT EXISTS idx_users_discord_id ON users(discord_id);');
}
