...
// Uploaded filenames are random and never reused (see utils/upload.js), so
// it's always safe to tell browsers to cache them hard instead of
// re-fetching the same image on every page load.
app.use(
  '/uploads',
  express.static(path.join(__dirname, '..', 'uploads'), { maxAge: '365d', immutable: true })
);
