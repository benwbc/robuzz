...
router.patch('/me/username', requireAuth, requireActive, async (req, res) => {
  ...
  // Same has_password exception as /me/password below — a Discord-only
  // account has no real password yet to confirm with.
  if (req.user.has_password && (!currentPassword || !bcrypt.compareSync(currentPassword, req.user.password_hash))) {
...
router.patch('/me/password', requireAuth, requireActive, async (req, res) => {
  ...
  // A Discord-created account starts with has_password = 0 — a random hash
  // sits in password_hash purely to satisfy the NOT NULL column, and no one
  // could ever know it, so there's nothing meaningful for "current password"
  // to check the first time. Every other account (has_password = 1) still
  // has to prove it knows the existing one, same as always.
  if (req.user.has_password && (!currentPassword || !bcrypt.compareSync(currentPassword, req.user.password_hash))) {
    return res.status(401).json({ error: 'Current password is incorrect.' });
  }
  ...
  await db.run('UPDATE users SET password_hash = ?, has_password = 1 WHERE id = ?', [hash, req.user.id]);
  res.json({ ok: true });
});
