...
if (usingCloudStorage) {
  ...
  if (bucketError) {
    console.error(
      `Supabase Storage bucket "${SUPABASE_BUCKET}" is not reachable (${bucketError.message}). ` +
        'Image uploads will fail until this is fixed. Check that SUPABASE_BUCKET exactly matches a bucket ' +
        'you created under Storage in Supabase, and that SUPABASE_SERVICE_KEY is the "secret" key from ' +
        'Settings → API Keys (not the "anon"/"publishable" one).'
    );
  } else {
    console.log(`Supabase Storage bucket "${SUPABASE_BUCKET}" is reachable.`);
  }
}
